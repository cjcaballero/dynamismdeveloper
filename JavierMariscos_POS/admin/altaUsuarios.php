<?php
    ...
    $puestoError = $nameError = $emailError = $usuarioError = $passwordError = $name = $email = $usuario = $password = "";
    ...
    if(!empty($_POST)) 
    {
        ...
        {
            $db = Database::connect();
            $statement = $db->prepare("INSERT INTO catusers (name,email,user,password,idpuesto,status) values(?, ?, ?, ?, ?)");
            $statement->execute(array($name,$email,$usuario,$password,$puesto,1));
            Database::disconnect();
            ...
            header("Location:usuarios.php?user=$user&return=$return");
        }
    }
                ...
                <form class="form" action="altausuarios.php?user=<?php echo $user ?>" class="form" role="form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                            ...
                            foreach ($db->query('SELECT * FROM catpuestos ORDER BY id DESC') as $row) 
                            {
